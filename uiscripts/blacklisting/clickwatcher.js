...
function Highlighter() {
  ...
  var enabled = false;
  var then = Date.now();
  var box = $("<div class='adremover-highlight-node'></div>");
  box.appendTo("body");
  ...
  function handler(e) {
    ...
    if (el === document.body || el.className === "adremover-killme-overlay") {
      box.hide();
      return;
    }
    el = $(el);
...
ClickWatcher.prototype._build_ui = function() {
  ...
  $("body").on("click", ".adremover-killme-overlay, .adremover-highlight-node",
    click_catch_this);
  ...
  var page = $("<div></div>").
    ...
    dialog({
      ...
      close: function() {
        $("body").off("click",
          ".adremover-killme-overlay, .adremover-highlight-node", click_catch_this);
        Overlay.removeAll();
        that._onClose();
        page.remove();
      },
