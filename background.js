  ...
  if (!SAFARI) {
    // Records how many ads have been blocked by adremover.  This is used
    // by the adremover app in the Chrome Web Store to display statistics
    // to the user.
    var blockCounts = (function() {
      ...
      return {
        recordOneadremovered: function(tabId) {
          var data = storage_get(key);
          data.total += 1;
          storage_set(key, data);
  ...
  var get_adremover_user_id = function() {
    return "0";
  };
  ...
  if (!SAFARI) {
    ...
    frameData = {
      ...
      track: function(details) {
        ...
        // Request from a tab opened before adremover started, or from a
    ...
    function onBeforeRequestHandler(details) {
      ...
      if (blocked){
        blockCounts.recordOneadremovered(tabId);
        updateBadge(tabId);
      }
  ...
  // Get or set if adremover is paused
  // Inputs: newValue (optional boolean): if true, adremover will be paused, if
  //                  false, adremover will not be paused.
  // Returns: undefined if newValue was specified, otherwise it returns true
  //          if paused, false otherwise.
  adremover_is_paused = function(newValue) {
    if (newValue === undefined) {
      return sessionStorage.getItem('adremover_is_paused') === "true";
    }
    sessionStorage.setItem('adremover_is_paused', newValue);
  }
  ...
  if (!SAFARI) {
    updateBadge = function(tabId) {
      ...
      if(display && (main_frame && isBlockable) && !adremover_is_paused()){
    ...
    updateButtonUIAndContextMenus = function() {
      ...
      function setContextMenus(info) {
        ...
        if (adremover_is_paused() || info.whitelisted || info.disabled_site)
          return;
  ...
  get_content_script_data = function(options, sender) {
    ...
    var runnable = !adremover_is_paused() && !page_is_unblockable(sender.tab.url);
  ...
  if (!SAFARI) {
    ...
    // doesn't hang Chrome while adremover initializes.
